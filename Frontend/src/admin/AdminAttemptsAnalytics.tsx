...
export default function AdminAttemptsAnalytics() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get<Track[]>("http://localhost:3000/api/assessments/tracks");
        setTracks(res.data || []);
      } catch {
  ...
  useEffect(() => {
    (async () => {
      ...
      setSelectedTest("");
      setStats(null);
      const res = await api.get<TestItem[]>(`http://localhost:3000/api/assessments/tracks/${selectedTrack}/tests`);
      setTests(res.data || []);
    })();
  }, [selectedTrack]);
  ...
  const loadStats = async () => {
    ...
    const res = await api.get(`http://localhost:3000/api/assessments/admin/tests/${selectedTrack}/${selectedTest}/attempts`);
    setStats(res.data);
  };
